...
const reqOptions = {
  headers: {
    'User-Agent': 'MinAnaGame/1.5 (aymanemadride09@gmail.com) Node.js'
  }
};
        ...
        try {
          if (res.statusCode === 200) {
            ...
            if (json.thumbnail && json.thumbnail.source) {
              let img = json.thumbnail.source.split('?')[0];
  ...
  for (const [catId, arr] of Object.entries(dbMap)) {
    ...
    for (const item of arr) {
      ...
      if (item.flag) {
        ...
        imageUrl = await getWikiImage(item.en);
        await delay(50); // fast! REST API is fast.
      }
