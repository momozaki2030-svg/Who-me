...
const reqOptions = {
  headers: {
    'User-Agent': 'MinAnaGame/2.0 (aymanemadride09@gmail.com) Node.js'
  }
};
  ...
  for (const chunk of chunks) {
    ...
    try {
      ...
      if (response) {
        ...
        // Map redirects to easily link original title to output
        const redirects = {};
        ...
        if (json.query.pages) {
          for (const pageId in json.query.pages) {
            ...
            if (page.thumbnail && page.thumbnail.source) {
              ...
              result[finalTitle] = page.thumbnail.source; // just in case
            }
          }
        }
      }
  ...
  for (const [catId, arr] of Object.entries(dbMap)) {
    ...
    for (const item of wikiItems) {
      ...
      if (!imageUrl) {
        ...
        // We will keep it but it might get filtered out by the validator, so let's give it a high-res generic image if absolutely needed, or null
    ...
    console.log(`Category ${catId} done. Missing images: ${missing}/${wikiItems.length}`);
  }
