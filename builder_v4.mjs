...
const reqOptions = {
  headers: {
    'User-Agent': 'MinAnaGame/1.2 (aymanemadride09@gmail.com) Node.js'
  }
};

async function fetchWikiEnImage(title) {
  try {
    ...
    const res2 = await fetch(searchUrl, reqOptions);
    const json2 = await res2.json();
    if (json2.query?.search?.length > 0) {
      const bestTitle = json2.query.search[0].title;
      ...
      const res3 = await fetch(imgUrl, reqOptions);
      const json3 = await res3.json();
      const pages3 = json3.query?.pages;
      if (pages3) {
        const pageId = Object.keys(pages3)[0];
        const img = pages3[pageId]?.thumbnail?.source;
        if (img) return img.split('?')[0];
      }
    }
  } catch (e) {
...
async function run() {
  ...
  for (const [catId, arr] of Object.entries(dbMap)) {
    ...
    for (const res of results) {
      finalItems.push({ id: `item_${counter++}`, ...res });
    }
  }
